...
func (d *Database) CreateGroup(juid types.JID, name string, code *string, sender string, msg string, err1 error, elapsedTime float64, participants int, num int, total int, startSend time.Time) error {
	erStr := ""
	if err1 != nil {
		erStr = err1.Error()
	}
...
func (d *Database) UpdateConfig(juid string, lastError string, totalGrupos int) error {
	...
	if totalGrupos == 0 {
		_, err := d.Conn.Exec("UPDATE config SET last_update = $1, last_error = $2 WHERE juid = $3", time.Now(), lastError, juid)
		if err != nil {
			println("ERRO NO UPDATE CONFIG", err)
		}
	} else {
		_, err := d.Conn.Exec(
